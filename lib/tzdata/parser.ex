defmodule Tzdata.Parser do
  ...
  def process_file(file_stream) do
    ...
    |> filter_empty_lines
    # Strip comments at line end. Like this comment.
  ...
  def process_tz_list([head | tail]) do
    ...
    case hd(split) do
      ...
      # pass through
      _ -> [head | process_tz_list(tail)]
    end
  end

  def process_rule(line) do
    rule_regex =
      ~r/Rule[\s]+(?<name>[^\s]+)[\s]+(?<from>[^\s]+)[\s]+(?<to>[^\s]+)[\s]+(?<type>[^\s]+)[\s]+(?<in>[^\s]+)[\s]+(?<on>[^\s]+)[\s]+(?<at>[^\s]+)[\s]+(?<save>[^\s]+)[\s]+(?<letter>[^\n]+)/
    ...
    captured = %{
      ...
      # we don't use this column for anything
      type: captured["type"],
  ...
  def zone_mapped(line) do
    ...
    zone_line_regex = [
      {:head_with_until,
       ~r/Zone[\s]+(?<name>[^\s]+)[\s]+(?<gmtoff>[^\s]+)[\s]+(?<rules>[^\s]+)[\s]+(?<format>[^\s]+)[\s]+(?<until>[^\n]+)/},
      {:head_no_until,
       ~r/Zone[\s]+(?<name>[^\s]+)[\s]+(?<gmtoff>[^\s]+)[\s]+(?<rules>[^\s]+)[\s]+(?<format>[^\s]+)/},
      {:continuation_with_until,
       ~r/[\s]+(?<gmtoff>[^\s]+)[\s]+(?<rules>[^\s]+)[\s]+(?<format>[^\s]+)[\s]+(?<until>[^\n]+)/},
      {:continuation_no_until,
       ~r/[\s]+(?<gmtoff>[^\s]+)[\s]+(?<rules>[^\s]+)[\s]+(?<format>[^\s]+)/}
    ]
  ...
  defp captured_zone_map_clean_up(captured) do
    ...
    Map.merge(
      %{
        gmtoff: string_amount_to_secs(captured["gmtoff"]),
